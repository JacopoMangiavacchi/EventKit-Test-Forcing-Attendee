...
class RootViewController: UITableViewController, EKEventEditViewDelegate {
    ...
    @IBAction func addEvent(_: AnyObject) {
        ...
        addController.event = createEventWithAttendees()
    ...
    private func createEventWithAttendees() -> EKEvent {
        let eventStore = self.eventStore!
        
        let event = EKEvent(eventStore: eventStore)
        event.title = "TEST EVENT"
        event.startDate = NSDate() as Date
        event.endDate = NSDate().addingTimeInterval(60 * 60) as Date
        event.calendar = self.defaultCalendar
        
        var attendees = [EKParticipant]()
        if let attendee = createParticipant(email: "kitchen@mangiavacchi.onmicrosoft.com") {
